...
add_hook('ShoppingCartValidateCheckout', 1, function ($vars) {
    try {
        ...
        // IP whitelist bypass -- admin/monitoring/CI IPs skip all fraud checks
        try {
            $wlRaw = Capsule::table('mod_fps_settings')
                ->where('setting_key', 'whitelisted_ips')
                ->value('setting_value');
            if (is_string($wlRaw) && $wlRaw !== '') {
                $wlIps = array_map('trim', explode(',', $wlRaw));
                if (in_array($ip, $wlIps, true)) {
                    return []; // Whitelisted IP, skip all checks
                }
            }
                ...
                if ($tsValidator->isEnabled() && $tsValidator->isFormProtected('checkout')) {
                    ...
                    if (!$tsResult['success'] && !in_array('network-error-failopen', $tsResult['error_codes'] ?? [])) {
                        ...
                        try {
                            ...
                            // Analytics server event (merged from main lineage; OFF by default).
        ...
        // 4b. Missing fingerprint penalty -- bots omit fps_fingerprint POST field
        if ($fingerprintData === '') {
            $score += 20;
            $providerScores['missing_fingerprint'] = 20.0;
            $details[] = 'No browser fingerprint submitted (possible bot)';
        }

        // 4c. User-Agent bot detection (instant, no API)
        $fpsUa = strtolower($_SERVER['HTTP_USER_AGENT'] ?? '');
        if ($fpsUa === '') {
            $score += 35;
            ...
            foreach ($fpsBotUaPatterns as $fpsUaPat) {
                if (strpos($fpsUa, $fpsUaPat) !== false) {
                    $score += 50;
                    $providerScores['user_agent'] = 50.0;
                    $details[] = 'Bot user agent detected';
                    break;
                }
            }
        }

        // 4d. AbuseSignalProvider (StopForumSpam + SpamHaus, no API key)
        if (($email !== '' || $ip !== '') && class_exists('\\FraudPreventionSuite\\Lib\\Providers\\AbuseSignalProvider')) {
            try {
                $abuseProvider = new \FraudPreventionSuite\Lib\Providers\AbuseSignalProvider();
                if ($abuseProvider->isEnabled()) {
                    $abuseResult = $abuseProvider->check($context);
                    $abuseContribution = (float) (($abuseResult['score'] ?? 0) * $abuseProvider->getWeight());
                    if ($abuseContribution > 0) {
                        $score += $abuseContribution;
                        $providerScores['abuse_signal'] = $abuseContribution;
                        if (!empty($abuseResult['details'])) {
                            $details = array_merge($details, (array) $abuseResult['details']);
                        }
                    }
                }
            } catch (\Throwable $e) { /* non-fatal */ }
...
add_hook('ClientAdd', 1, function ($vars) {
    try {
        ...
        if (class_exists('\\FraudPreventionSuite\\Lib\\Models\\FpsCheckContext')
            ...
            // Analytics high-risk-signup server event (merged from main; OFF by default).
            if (class_exists(FpsAnalyticsServerEvents::class)) {
                try {
                    $highRiskThreshold = (float) FpsAnalyticsConfig::get('analytics_high_risk_signup_threshold', '80');
                    if ($result->risk->score >= $highRiskThreshold) {
...
// ---------------------------------------------------------------------------
// FPS Analytics (merged from main lineage v4.2.6)
// Page injectors + consent banner + daily anomaly detection. All guarded by
// class_exists() + FpsAnalyticsConfig gates; every analytics setting defaults
// OFF, so these hooks no-op until an operator opts in via the Analytics card.
// ---------------------------------------------------------------------------

// Admin-side GA4 / Clarity injection (FPS admin tabs).
add_hook('AdminAreaHeaderOutput', 5, function ($vars) {
    try {
        if (!class_exists(FpsAnalyticsInjector::class) || !FpsAnalyticsConfig::isAdminEnabled()) {
            return '';
        }
        $adminId   = (int) ($_SESSION['adminid'] ?? 0);
        $adminRole = (string) ($vars['rolename'] ?? '');
        return FpsAnalyticsInjector::admin($adminId, $adminRole);
    } catch (\Throwable $e) {
        logModuleCall('fraud_prevention_suite', 'AnalyticsInject::Admin', '', $e->getMessage());
        return '';
    }
});

// Client-side GA4 / Clarity injection + consent banner (storefront + cart).
add_hook('ClientAreaFooterOutput', 5, function ($vars) {
    try {
        ...
        $visitorCountry = '';
        $ip = $_SERVER['REMOTE_ADDR'] ?? '';
        if ($ip !== '' && method_exists('\FraudPreventionSuite\Lib\FpsHookHelpers', 'fps_lookupCountryByIp')) {
        ...
        logModuleCall('fraud_prevention_suite', 'AnalyticsInject::Client', '', $e->getMessage());
        return '';
    }
});

// Daily analytics maintenance: anomaly detection, log purge, health heartbeat.
add_hook('DailyCronJob', 5, function ($vars) {
    ...
    if (class_exists('\FraudPreventionSuite\Lib\Analytics\FpsAnalyticsLog')) {
        try { \FraudPreventionSuite\Lib\Analytics\FpsAnalyticsLog::purgeOlderThan(30); }
        catch (\Throwable $e) { /* non-fatal */ }
    }
    if (class_exists(FpsAnalyticsServerEvents::class)) {
        try {
